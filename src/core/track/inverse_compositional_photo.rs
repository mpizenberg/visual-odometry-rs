...
use crate::core::{
    ...
    multires,
    track::lm_optimizer_photo::{self as lm_optimizer, LMOptimizerState},
};
    ...
    pub fn init(
        ...
        Tracker {
            state: State {
                ...
                current_frame_exposure: (0.0, 0.0),
            },
            config: self,
        }
    }
...
impl Tracker {
    ...
    pub fn track(
        ...
        let motion = self.state.current_frame_pose.inverse() * self.state.keyframe_pose;
        let (photo_a, photo_b) = self.state.current_frame_exposure;
        let mut lm_model = (motion, photo_a, photo_b);
        ...
        for lvl in (0..self.config.nb_levels).rev() {
            let obs = lm_optimizer::Obs {
                ...
                template_exposure: &self.state.keyframe_exposure,
            };
        ...
        let optical_flow_sum: Float = _z_candidates
            ...
            .map(|(&_z, &(x, y))| {
                let (u, v) = warp(&motion, x as Float, y as Float, _z, intrinsics);
